...
export default function EnergyWisePage() {
  ...
  >("disconnected");
  const [connectionType, setConnectionType] = useState<"rs485" | "wifi">(
    "rs485",
  );
  ...
  const handleConnect = () => {
    ...
    setTimeout(() => {
      ...
      setShowConfig(false);
      setPotenciaTotal(
        INITIAL_INVERSORES.reduce((acc, inv) => acc + inv.potenciaAtual, 0),
      );
    }, 2000);
  };
    ...
    intervalRef.current = setInterval(() => {
      setInversores((prev) =>
        prev.map((inv) => {
          ...
          const novaCorrente =
            (novaPotencia * 1000) / (Math.sqrt(3) * inv.tensaoCA);
          return {
            ...inv,
  ...
  useEffect(() => {
    if (connectionStatus === "connected") {
      setPotenciaTotal(
        inversores.reduce((acc, inv) => acc + inv.potenciaAtual, 0),
      );
    }
  }, [inversores, connectionStatus]);

  const totalGeracaoDia = inversores.reduce(
    (acc, inv) => acc + inv.geracaoDia,
    0,
  );
          ...
          >
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              <h2 className="text-lg font-bold text-slate-800 mb-6">
                Configuração do Datalogger / Gateway
              </h2>
              <div className="flex gap-4 mb-6">
                ...
                >
                  ...
                  <span className="text-xs text-center opacity-70">
                    Leitura Modbus RTU em tempo real (1s)
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="text-xs text-center opacity-70">
                    Integração via portal WEG/Fronius (Delay 15m)
                  </span>
                </button>
              </div>
              <div className="bg-slate-50 p-6 rounded-xl border border-slate-100 mb-6">
                {connectionType === "wifi" ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">
                        ID da Planta (Plant ID)
                      </label>
                      <input
                        type="text"
                        defaultValue="PLANT-99823"
                        className="w-full p-2 border rounded-lg bg-white"
                      />
                    </div>
                  </div>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <Server className="text-blue-500" /> Inversores da Usina
              </h2>
            </div>
