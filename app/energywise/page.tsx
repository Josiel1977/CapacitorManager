...
import { 
  Sun, Activity, Zap, Wifi, Cable, Server, Settings,
  CheckCircle2, AlertCircle, RefreshCw, TrendingUp,
  Calendar, DollarSign, Factory, MapPin
} from 'lucide-react';
import { 
...
interface Inversor {
  ...
  modelo: string;
  sn: string;
  status: 'Produzindo' | 'Offline' | 'Alerta';
...
// ===================== MOCKS (substituir por chamadas Supabase) =====================
const MOCK_CURVA_GERACAO = [
  { hora: '06:00', kw: 0 }, { hora: '07:00', kw: 2.5 }, { hora: '08:00', kw: 12.4 },
  { hora: '09:00', kw: 25.8 }, { hora: '10:00', kw: 38.2 }, { hora: '11:00', kw: 45.5 },
  { hora: '12:00', kw: 50.2 }, { hora: '13:00', kw: 48.1 }, { hora: '14:00', kw: 42.0 },
  { hora: '15:00', kw: 30.5 }, { hora: '16:00', kw: 18.2 }, { hora: '17:00', kw: 5.4 },
  { hora: '18:00', kw: 0.5 }, { hora: '19:00', kw: 0 }
];

const MOCK_GERACAO_MENSAL = Array.from({ length: 15 }, (_, i) => ({
  dia: `${i + 1}/04`,
  kwh: Math.floor(Math.random() * 100) + 200
}));

const INITIAL_INVERSORES: Inversor[] = [
  { 
    id: 'inv1', modelo: 'WEG SIW500H ST030 M3', sn: '6T2189045432', status: 'Produzindo',
    potenciaAtual: 25.4, tensaoCA: 380, correnteCA: 38.5, geracaoDia: 112.5, temperatura: 45
  },
  { 
    id: 'inv2', modelo: 'WEG SIW500H ST030 M3', sn: '6T2189045433', status: 'Produzindo',
    potenciaAtual: 24.8, tensaoCA: 381, correnteCA: 37.6, geracaoDia: 110.2, temperatura: 46
  }
];
...
export default function EnergyWisePage() {
  ...
  const [inversores, setInversores] = useState<Inversor[]>(INITIAL_INVERSORES);
  
  // Configuração salva no localStorage
  const [config, setConfig] = useState<ConfigConexao>(() => {
    const saved = localStorage.getItem('energywise_config');
    if (saved) return JSON.parse(saved);
    return { tipo: 'rs485', ip: '192.168.1.150', porta: 502, baudRate: 19200 };
  });
  ...
  useEffect(() => {
    localStorage.setItem('energywise_config', JSON.stringify(config));
  }, [config]);
  ...
  // Atualiza potência total quando inversores mudam
  useEffect(() => {
  ...
  const economiaEstimada = totalGeracaoDia * 0.95;
    ...
    <div className="space-y-8 pb-12 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-slate-400 text-sm mt-1">
              <Factory size={14} /> <span>Indústria XYZ</span>
              <span className="text-slate-600">•</span>
              <MapPin size={14} /> <span>Usina Telhado Principal</span>
            </div>
          </div>
        </div>
          ...
          >
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <div className="flex gap-4 mb-6">
                <button
                  onClick={() => setConfig(c => ({ ...c, tipo: 'rs485' }))}
                  className={cn(
                    "flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all",
                    config.tipo === 'rs485' ? "border-yellow-500 bg-yellow-50 text-yellow-700" : "border-slate-100 bg-slate-50 text-slate-500"
                  )}
                >
                  <Cable size={24} />
                ...
                <button
                  onClick={() => setConfig(c => ({ ...c, tipo: 'wifi' }))}
                  className={cn(
                    "flex-1 flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all",
                    config.tipo === 'wifi' ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-100 bg-slate-50 text-slate-500"
                  )}
                >
                  <Wifi size={24} />
                ...
                {config.tipo === 'wifi' ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select 
                        ...
                        value={config.fabricante || 'WEG'}
                        onChange={e => setConfig(c => ({ ...c, fabricante: e.target.value }))}
                      >
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        onChange={e => setConfig(c => ({ ...c, plantId: e.target.value }))}
                        className="w-full p-2 border rounded-lg bg-white" 
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <input 
                        type="number" 
                        value={config.porta || 502}
                        onChange={e => setConfig(c => ({ ...c, porta: parseInt(e.target.value) }))}
                        className="w-full p-2 border rounded-lg bg-white" 
                      />
                    </div>
                    <div>
      ...
      {connectionStatus === 'connected' && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
              <div className="flex items-center gap-3 mb-2">
                <div className="rounded-lg bg-yellow-50 p-2 text-yellow-600"><Zap size={20} /></div>
              ...
              <p className="text-xs text-green-600 font-medium mt-2 flex items-center gap-1"><Activity size={12} /> Atualizado agora</p>
            </div>

            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
              <div className="flex items-center gap-3 mb-2">
                <div className="rounded-lg bg-blue-50 p-2 text-blue-600"><Sun size={20} /></div>
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
              <div className="flex items-center gap-3 mb-2">
                <div className="rounded-lg bg-purple-50 p-2 text-purple-600"><Calendar size={20} /></div>
            ...
            <div className="rounded-2xl bg-green-50 p-6 shadow-sm border border-green-100">
              <div className="flex items-center gap-3 mb-2">
                <div className="rounded-lg bg-green-100 p-2 text-green-700"><DollarSign size={20} /></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
              <div className="flex items-center justify-between mb-6">
                <div><h2 className="text-lg font-bold text-slate-800">Curva de Geração</h2><p className="text-sm text-slate-500">Potência (kW) ao longo do dia</p></div>
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={MOCK_CURVA_GERACAO} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <defs><linearGradient id="colorKw" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#EAB308" stopOpacity={0.3}/><stop offset="95%" stopColor="#EAB308" stopOpacity={0}/></linearGradient></defs>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                    <XAxis dataKey="hora" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} dy={10} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} />
                    <RechartsTooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} formatter={(value: any) => [`${value} kW`, 'Potência']} />
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
              <div className="flex items-center justify-between mb-6">
                <div><h2 className="text-lg font-bold text-slate-800">Histórico de Energia</h2><p className="text-sm text-slate-500">Energia (kWh) gerada por dia</p></div>
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={MOCK_GERACAO_MENSAL} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} />
                    <RechartsTooltip cursor={{ fill: '#f8fafc' }} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} formatter={(value: any) => [`${value} kWh`, 'Energia']} />
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2"><Server className="text-blue-500" /> Inversores da Usina</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead><tr className="border-b border-slate-100 text-sm font-medium text-slate-500">
                  <th className="pb-4">Equipamento</th><th className="pb-4">Status</th><th className="pb-4">Potência (kW)</th>
                  <th className="pb-4">Tensão (V)</th><th className="pb-4">Corrente (A)</th><th className="pb-4">Temp. (°C)</th><th className="pb-4">Gerado Hoje (kWh)</th>
                </tr></thead>
                <tbody className="divide-y divide-slate-50">
                  {inversores.map((inv) => (
                    <tr key={inv.id} className="text-sm text-slate-700">
                      <td className="py-4"><p className="font-bold text-slate-900">{inv.modelo}</p><p className="text-xs text-slate-500 font-mono">SN: {inv.sn}</p></td>
                      <td className="py-4"><span className="inline-flex items-center gap-1.5 rounded-full bg-green-50 px-2.5 py-1 text-xs font-bold text-green-700"><span className="h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse" />{inv.status}</span></td>
                      ...
                      <td className="py-4"><span className={cn("px-2 py-1 rounded-md text-xs font-bold", inv.temperatura > 60 ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-600")}>{inv.temperatura}°C</span></td>
