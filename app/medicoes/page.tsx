...
function MedicoesContent() {
      ...
      const processedMedicoes = (medData || []).map((med) => {
        ...
        if (capData) {
          ...
          if (med.tipo_teste === "corrente" && med.corrente_medida_a) {
            ...
            const capacitanciaTeorica = calcularCapacitanciaTeorica(
              capData.capacitancia_nominal_uf,
              ligacao,
            );
  ...
  async function downloadPDF() {
    ...
    try {
      ...
      pdf.save(
        `Medicoes_${capacitor?.codigo_identificacao || "capacitor"}.pdf`,
      );
  ...
  const aprovados = medicoes.filter(
    (m) => m.status_validacao === "aprovado",
  ).length;
  const atencao = medicoes.filter(
    (m) => m.status_validacao === "atencao",
  ).length;
  const reprovados = medicoes.filter(
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500">
              Capacitor: {capacitor.codigo_identificacao} -{" "}
        ...
        <div className="flex gap-2">
          ...
          >
            <Activity
              size={16}
              className={recalculando ? "animate-spin" : ""}
            />
            Recalcular
          </button>
          {medicoes.length > 0 && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="text-center">
            ...
            <p className="text-xl font-bold text-primary">
              {capacitor.potencia_kvar} kVAr
            </p>
          </div>
          <div className="text-center">
            <p className="text-xs text-slate-500">Tensão</p>
            <p className="text-xl font-bold text-primary">
              {capacitor.tensao_nominal_v} V
            </p>
          </div>
          <div className="text-center">
      ...
      >
        ...
        <div className="text-center p-8 pb-4 border-b">
          ...
          <p className="text-slate-500">
            Banco: {capacitor.bancos_capacitores?.nome_banco}
          </p>
          <p className="text-slate-500">
        ...
        {medicoes.length > 0 ? (
          <div className="overflow-x-auto p-6">
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        ...
                        {med.desvio_percentual?.toFixed(2)}%
                      </span>
                    </td>
                    <td className="px-4 py-3">
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 text-xs text-slate-400">
              <p className="font-bold mb-2">
                📌 Critérios de Validação - ANEXO TÉCNICO:
              </p>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <ul className="space-y-1 list-disc list-inside ml-4">
                    ...
                    <li>
                      <strong className="text-amber-600">⚠️ Atenção:</strong>{" "}
                      Desvio entre -10% e -5% OU entre +10% e +15%
                    </li>
                    <li>
                      <strong className="text-red-600">❌ Reprovado:</strong>{" "}
                      Desvio abaixo de -10% ou acima de +15%
                    </li>
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <ul className="space-y-1 list-disc list-inside ml-4">
                    <li>
                      ...
                      I = Potência(kVAr) × 1000 / (√3 × Tensão Nominal)
                    </li>
                    <li>
                      <strong className="text-blue-600">
                        Tensão utilizada:
                      </strong>{" "}
                      {capacitor?.tensao_nominal_v} V (nominal do capacitor, NÃO
                      a tensão medida)
                    </li>
                    <li>
                ...
                <div className="bg-slate-50 p-2 rounded-lg mt-2">
                  <p className="text-slate-500">
                    💡 Importante: A validação utiliza a{" "}
                    <strong className="text-blue-600">
                      Tensão Nominal do Capacitor
                    </strong>{" "}
                    (etiqueta do equipamento), independente da tensão medida no
                    momento do teste.
                  </p>
                </div>
              </div>
          ...
          <div className="text-center py-12 text-slate-400">
            ...
            <button
              onClick={() =>
                router.push(`/testes?capacitor_id=${capacitor.id}`)
              }
              className="mt-4 text-primary hover:underline"
            >
