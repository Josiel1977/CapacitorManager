...
interface Fatura {
  ...
  tenant_id?: string;
  fp_calculado?: number; // fator de potência lido diretamente do PDF
}
...
// ==================== PDF PARSER (regex robustos estilo Python) ====================
...
function DimensionarContent() {
  ...
  // Autenticação e carregamento de dados (original)
  useEffect(() => {
    let mounted = true;
  ...
  const calcularDimensionamento = () => {
    ...
    try {
      ...
      // Processa cada fatura: calcula ativo, reativo, FP (prioriza o lido) e demanda máxima
      ...
      console.log(`📈 Demanda real máxima: ${demandaReal} kW | Pior FP: ${fpAtual}`);

      // Força uma demanda mínima razoável caso a extraída seja zero (ex.: fallback)
      if (demandaReal < 10 && potenciaTotalTransformadores > 0) {
        demandaReal = potenciaTotalTransformadores * 0.2;
        alertas.push(`⚠️ Demanda extraída muito baixa. Utilizado valor estimado de ${demandaReal.toFixed(1)} kW.`);
      }
      ...
      if (precisaCapacitor) {
        // Cálculo teórico exato conforme fórmula de compensação
        ...
        // Arredondamento comercial com base no valor teórico (igual ao algoritmo Python)
        let baseStep = 2.5;
        ...
        motivo = `Potência ativa = ${potenciaAtivaFinal.toFixed(1)} kW | FP atual = ${(fpAtual*100).toFixed(1)}% | Meta = ${(fpDesejado*100).toFixed(0)}% → kVAr teórico = ${kvarTeorico.toFixed(1)} → comercial = ${kvarAutomatico.toFixed(1)} kVAr (${estagios.length} estágios).`;
      } else {
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { setCurrentFatura({}); setEditandoFaturaId(null); setShowFaturaModal(true); }} className="text-xs bg-primary text-white px-3 py-1 rounded-lg">
                  ...
                  <div key={fat.id} className="p-3 rounded-lg bg-slate-50">
                    ...
                    <div className="grid grid-cols-2 gap-1 text-xs mt-2">
                      ...
                      <div className="col-span-2">
                        <span className={`text-xs font-bold ${fp >= 0.92 ? "text-green-600" : "text-red-600"}`}>FP: {(fp * 100).toFixed(1)}%</span>
                  ...
                  {result.precisa_capacitor ? (
                    <>
                      ...
                      <div className="bg-blue-50 p-4 rounded-xl">
                        ...
                        <div className="mt-3"><BarraFP fp={result.fp_atual_percent} /><div className="flex justify-between text-[10px] mt-1"><span>Atual: {result.fp_atual_percent.toFixed(1)}%</span><span>Meta ANEEL: 92%</span></div></div>
                      </div>
                      <div className="bg-slate-50 rounded-xl p-4">
                        <p className="text-xs font-bold flex gap-2"><Activity size={14} /> Evolução do FP e Multa por Mês</p>
                        <div className="space-y-2 max-h-48 overflow-y-auto">
                          {result.media_fp_por_mes.map((item, idx) => (
                            <div key={idx} className="flex items-center gap-2 text-xs">
                              <span className="w-14 font-medium">{item.mes}</span>
                              <div className="flex-1"><div className="w-full bg-slate-200 rounded-full h-1.5"><div className={`${item.fp >= 92 ? "bg-green-500" : item.fp >= 80 ? "bg-amber-500" : "bg-red-500"} h-1.5 rounded-full`} style={{ width: `${Math.min(100, item.fp)}%` }} /></div></div>
                      ...
                      {result.pior_mes && (
                        <div className="bg-amber-50 p-4 rounded-xl">
                          ...
                          <p className="text-sm mt-1">FP: {result.pior_mes.fp_calculado ? (result.pior_mes.fp_calculado * 100).toFixed(1) : "0"}% • Multa: {formatMoney(calcularMultaDaFatura(result.pior_mes as Fatura))}</p>
                        </div>
                      )}
                      <div className="bg-indigo-50 p-4 rounded-xl">
                        <p className="text-xs font-bold flex gap-2"><Factory size={14} /> Distribuição do Banco entre Transformadores</p>
                        {result.distribuicao_por_trafo.map((dist, idx) => (
                          <div key={idx} className="bg-white rounded-lg p-3 mt-2 border">
                            <div className="flex justify-between"><span className="font-bold text-sm">Transformador {formatNumber(dist.trafo_kva, 0)} kVA</span><span className="text-xs text-slate-500">{dist.percentual.toFixed(1)}% da carga</span></div>
                      ...
                      <div className="bg-emerald-50 p-4 rounded-xl">
                        <p className="text-xs font-bold flex gap-2"><DollarSign size={14} /> Análise Financeira Real</p>
                        <div className="grid grid-cols-2 gap-2 text-center mt-2">
                          <div className="bg-white rounded p-2 border"><p className="text-[10px] text-slate-500">Investimento Total</p><p className="font-bold text-lg">{formatMoney(result.investimento_estimado_total)}</p></div>
                          <div className="bg-white rounded p-2 border"><p className="text-[10px] text-slate-500">Custo por kVAr</p><p className="font-bold">{formatMoney(result.preco_por_kvar)}/kVAr</p></div>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-center mt-2">
                          <div className="bg-white rounded p-2 border"><p className="text-[10px] text-slate-500">Payback</p><p className="font-bold text-green-600">{result.payback_meses} meses</p></div>
                          <div className="bg-white rounded p-2 border"><p className="text-[10px] text-slate-500">Economia/ano</p><p className="font-bold">{formatMoney(result.economia_anual)}</p></div>
                          <div className="bg-white rounded p-2 border"><p className="text-[10px] text-slate-500">Retorno 5 anos</p><p className={`font-bold ${result.retorno_5_anos > 0 ? "text-green-700" : "text-red-700"}`}>{formatMoney(result.retorno_5_anos)}</p></div>
                        </div>
                      </div>
