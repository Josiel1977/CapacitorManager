...
function parseFaturaFromPDF(
  ...
  const consumoPontaMatch = texto.match(
    /TUSD\s*Energia\s*Ponta\s*\(kWh\)\s*.*?([\d\.]+,\d+)/i,
  );
  if (consumoPontaMatch)
    dados.consumo_ponta_kwh = parseFloat(
      consumoPontaMatch[1].replace(/\./g, "").replace(",", "."),
    );

  const consumoForaPontaMatch = texto.match(
    /TUSD\s*Energia\s*Fora\s*Ponta\s*\(kWh\)\s*.*?([\d\.]+,\d+)/i,
  );
  if (consumoForaPontaMatch)
    dados.consumo_fora_ponta_kwh = parseFloat(
      consumoForaPontaMatch[1].replace(/\./g, "").replace(",", "."),
    );
  ...
  const demIsentaMatch = texto.match(
    /Demanda\s*Distrib\.\s*Isenta\s*ICMS\s*\(kW\)\s*.*?([\d\.]+,\d+)/i,
  );
  if (demIsentaMatch)
    demandaIsenta = parseFloat(
      demIsentaMatch[1].replace(/\./g, "").replace(",", "."),
    );

  const demNormalMatch = texto.match(
    /Demanda\s*Distribui[çc][ãa]o\s*\(kW\)\s*.*?([\d\.]+,\d+)/i,
  );
  if (demNormalMatch)
    demandaNormal = parseFloat(
      demNormalMatch[1].replace(/\./g, "").replace(",", "."),
    );
  ...
  // Reativo Ponta
  const reativoPontaMatch = texto.match(
    /Reativo\s*Excedente\s*P\s*\(?kVArh?\)?,*.*?([\d\.]+,\d+)\s+.*?([\d\.]+,\d{2})/i,
  );
  if (reativoPontaMatch) {
    dados.reativo_ponta_kvarh = parseFloat(
      reativoPontaMatch[1].replace(/\./g, "").replace(",", "."),
    );
    totalMultaReais += parseFloat(
      reativoPontaMatch[2].replace(/\./g, "").replace(",", "."),
    );
  }

  // Reativo Fora Ponta
  const reativoForaPontaMatch = texto.match(
    /Reativo\s*Excedente\s*NP\s*\(?kVArh?\)?,*.*?([\d\.]+,\d+)\s+.*?([\d\.]+,\d{2})/i,
  );
  if (reativoForaPontaMatch) {
    dados.reativo_fora_ponta_kvarh = parseFloat(
      reativoForaPontaMatch[1].replace(/\./g, "").replace(",", "."),
    );
    totalMultaReais += parseFloat(
      reativoForaPontaMatch[2].replace(/\./g, "").replace(",", "."),
    );
  }
  ...
  if (!valorMatch)
    valorMatch = texto.match(/Valor\s*cobrado\s*\(R\$\):\s*([\d\.]+,\d{2})/i);
  if (valorMatch)
        ...
        const faturasRaw = [
          {
            ...
            demanda_ponta_kw: 293.44,
            demanda_fora_ponta_kw: 293.44,
          ...
          },
          {
            ...
            demanda_ponta_kw: 280.0,
            demanda_fora_ponta_kw: 280.0,
            ...
            concessionaria: "EQUATORIAL_PARA",
            tenant_id: tenant,
            fator_potencia: 0.3677,
            multa_reativo_reais: 430.0,
          },
          {
            id: crypto.randomUUID(),
    ...
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        Swal.fire(
          "Erro na leitura",
          err.message ||
            "Não foi possível interpretar o PDF. Verifique se é uma fatura de energia.",
          "error",
        );
      } finally {
        setImportandoPDF(false);
  ...
  const calcularDimensionamento = () => {
    ...
    try {
      ...
      let faturasProcessadas = faturas.map((f) => {
        ...
        let fp =
          f.fator_potencia || calcularFatorPotencia(ativoTotal, reativoTotal);
      ...
      let demandaReal = Math.max(
        ...faturasProcessadas.map((f) => f.demandaMaxKw),
        0,
      );

      let potenciaBase =
        demandaPersonalizadaKw > 0 ? demandaPersonalizadaKw : demandaReal;
      let potenciaAtivaFinal = potenciaBase * (1 + margemSeguranca / 100);

      const precisaCapacitor =
        fpAtual < FP_MINIMO_REGULAMENTAR || mediaMulta > 50;
      ...
      const precoPorKvar =
        kvarAutomatico > 0 ? calcularPrecoMercado(kvarAutomatico) : 0;
      const custoTotalInstalacao = precoPorKvar * 1.25; // 25% para cubículo, proteções e mão de obra
      const payback =
        economiaMensal > 0 ? custoTotalInstalacao / economiaMensal : 0;
      ...
      const res: ResultadoDimensionamento = {
        ...
        pior_mes: piorMes,
        media_fp_por_mes: faturasProcessadas.map((f) => ({
          mes: f.mes_referencia,
          fp: f.fp * 100,
          multa: f.multa,
        })),
        alertas,
        distribuicao_por_trafo: distribuirKvarPorTrafo(
          transformadores,
          estagios,
          kvarAutomatico,
        ),
        ...
        roi_5_anos_percent:
          custoTotalInstalacao > 0
            ? (retorno5Anos / custoTotalInstalacao) * 100
            : 0,
        metodo_calculo_utilizado:
          "Análise Avançada Baseada em Demandas Totais de Fatura",
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
        <h2 className="text-xl font-bold flex items-center gap-2 mb-4">
          <Factory className="text-primary" /> Análise de Subestação e Faturas
        </h2>
        <div
          {...getRootProps()}
        ...
        >
          ...
          <p className="text-sm text-gray-600">
            Arraste ou clique para importar o PDF das faturas de energia
          </p>
        </div>
        <button
          onClick={calcularDimensionamento}
          className="mt-4 bg-blue-600 text-white px-4 py-2 rounded shadow hover:bg-blue-700 w-full flex justify-center gap-2"
        >
          {calculando ? <Loader2 className="animate-spin" /> : <Calculator />}{" "}
          Calcular Dimensionamento do Banco
        </button>
      </div>
    </div>
  );
