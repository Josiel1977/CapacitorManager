...
const distribuirKvarPorTrafo = (
  ...
  return transformadores.map((trafo) => {
    ...
    estagiosProporcionais = estagiosProporcionais.map((s) =>
      Math.max(2.5, Math.ceil(s / 2.5) * 2.5),
    );
    ...
    if (Math.abs(diff) > 0.01) {
      ...
      estagiosProporcionais[estagiosProporcionais.length - 1] = Math.max(
        2.5,
        Math.ceil(
          estagiosProporcionais[estagiosProporcionais.length - 1] / 2.5,
        ) * 2.5,
      );
    }
    const configuracao =
      estagiosProporcionais.map((s) => `${s.toFixed(1)}`).join(" + ") + " kVAr";
...
function parseFaturaFromPDF(
  ...
  if (texto.includes("Equatorial Pará"))
    dados.concessionaria = "EQUATORIAL_PARA";
  ...
  for (const match of kWhMatches) {
    ...
    if (valor > (dados.consumo_fora_ponta_kwh || 0)) {
      dados.consumo_fora_ponta_kwh = valor;
    } else if (
      valor > 0 &&
      (dados.consumo_ponta_kwh === 0 || valor < dados.consumo_fora_ponta_kwh)
    ) {
      dados.consumo_ponta_kwh = valor;
    }
  }
  ...
  for (const match of kVArhMatches) {
    let valor = parseFloat(match[1].replace(/\./g, "").replace(",", "."));
    if (valor > (dados.reativo_fora_ponta_kvarh || 0)) {
      dados.reativo_fora_ponta_kvarh = valor;
    } else if (
      valor > 0 &&
      (dados.reativo_ponta_kvarh === 0 ||
        valor < dados.reativo_fora_ponta_kvarh)
    ) {
      dados.reativo_ponta_kvarh = valor;
    }
  }

  // 🔥 DEMANDA CORRIGIDA – busca exata pelos campos da fatura
  // Exemplo: "Dem. Máx. F. Ponta (kW): 39,98"   e   "Dem. Máx. Ponta (kW): 37,97"
  const demFPMax = texto.match(
    /Dem\.\s*Máx\.\s*F\.\s*Ponta\s*\(kW\):\s*(\d+[\.,]?\d*)/i,
  );
  if (demFPMax)
    dados.demanda_fora_ponta_kw = parseFloat(demFPMax[1].replace(",", "."));

  const demPMax = texto.match(
    /Dem\.\s*Máx\.\s*Ponta\s*\(kW\):\s*(\d+[\.,]?\d*)/i,
  );
  if (demPMax)
    dados.demanda_ponta_kw = parseFloat(demPMax[1].replace(",", "."));

  // Fallback robusto: estima demanda pelo consumo total (considera 200 horas/mês)
  const consumoTotal = dados.consumo_ponta_kwh + dados.consumo_fora_ponta_kwh;
  if (
    consumoTotal > 0 &&
    dados.demanda_fora_ponta_kw === 0 &&
    dados.demanda_ponta_kw === 0
  ) {
    const horasMes = 200; // média para industrial/comercial
    const potenciaMedia = consumoTotal / horasMes;
    dados.demanda_fora_ponta_kw = Math.round(potenciaMedia * 1.2); // margem de pico
    dados.demanda_ponta_kw = Math.round(potenciaMedia * 0.8);
  }

  // Valor total
  let valorMatch = texto.match(/Valor cobrado\s*\(R\$\):\s*([\d\.]+,\d{2})/i);
  if (!valorMatch)
    valorMatch = texto.match(/Total a Pagar\s*R\$\s*([\d\.]+,\d{2})/i);
  if (valorMatch)
    dados.total_pagar = parseFloat(
...
function DimensionarContent() {
  ...
  const salvarTransformadores = async () => {
    if (!tenantId) return;
    const { error } = await supabase.from("transformadores").upsert(
      transformadores.map((t) => ({ ...t, tenant_id: tenantId })),
      { onConflict: "id" },
    );
    error
  ...
  const salvarFatura = async () => {
    if (!currentFatura.mes_referencia)
      return Swal.fire("Atenção", "Mês/ano obrigatório", "warning");
    if (!tenantId) return;
    const novaFatura = {
    ...
    const { error } = await supabase
      .from("faturas")
      .upsert(novaFatura, { onConflict: "id" });
    ...
    novas.sort(
      (a, b) =>
        parseMesReferencia(b.mes_referencia) -
        parseMesReferencia(a.mes_referencia),
    );
    setFaturas(novas);
  ...
  const onDropPDF = useCallback(
    async (acceptedFiles: File[]) => {
      if (!tenantId)
        return Swal.fire("Erro", "Tenant não identificado.", "error");
      ...
      try {
        ...
        const novaFatura: any = {
          ...
          reativo_fora_ponta_kvarh:
            dadosExtraidos.reativo_fora_ponta_kvarh || 0,
        ...
        Swal.fire(
          "✅ Sucesso!",
          `Fatura ${novaFatura.mes_referencia} importada com sucesso.`,
          "success",
        );
      } catch (err: any) {
        console.error(err);
        Swal.fire(
          "Erro na leitura",
          err.message ||
            "Não foi possível interpretar o PDF. Verifique se é uma fatura de energia (Equatorial, CELG, etc.)",
          "error",
        );
      } finally {
        setImportandoPDF(false);
  ...
  try {
    ...
    // 🔥 Obtém a maior demanda real entre todas as faturas (pontas e fora ponta)
    let demandaReal = Math.max(...faturasProcessadas.map((f) => f.demandaMaxKw), 0);

    console.log("📈 Demanda real máxima extraída das faturas:", demandaReal, "kW");

    // 🔥 Se a demanda real for muito baixa (< 30 kW) e houver transformadores de grande porte (≥ 225 kVA),
    // estima a demanda pelo consumo médio mensal (considerando 200 horas/mês)
    const potenciaInstaladaKVA = potenciaTotalTransformadores;
    if (demandaReal < 30 && potenciaInstaladaKVA >= 225) {
      const consumoMedioMensal = faturasProcessadas.reduce((acc, f) => acc + f.ativoTotal, 0) / faturasProcessadas.length;
      const horasEstimadas = 200; // média para comercial/industrial
      const demandaEstimada = consumoMedioMensal / horasEstimadas;
      console.log(`⚠️ Demanda real (${demandaReal.toFixed(1)} kW) muito baixa para a potência instalada (${potenciaInstaladaKVA} kVA). Demanda estimada pelo consumo: ${demandaEstimada.toFixed(1)} kW`);
      demandaReal = Math.max(demandaReal, demandaEstimada);
      alertas.push(`⚠️ Demanda registrada muito baixa (${demandaReal.toFixed(1)} kW). Utilizada estimativa baseada no consumo médio (${demandaEstimada.toFixed(1)} kW).`);
    }

    // 🔥 Garantir um mínimo razoável para clientes do Grupo A (transformadores ≥ 75 kVA)
    if (demandaReal < 40 && potenciaInstaladaKVA >= 75) {
      demandaReal = 50;
      alertas.push(`⚠️ Demanda mínima ajustada para 50 kW (valor conservador para Grupo A).`);
    }

    // Usa demanda personalizada se preenchida
    ...
    console.log(`✅ Potência ativa utilizada no cálculo: ${potenciaBase.toFixed(1)} kW (margem: ${margemSeguranca}%) -> final: ${potenciaAtivaFinal.toFixed(1)} kW`);
    ...
    if (precisaCapacitor) {
      ...
      motivo = `Potência ativa utilizada = ${potenciaAtivaFinal.toFixed(1)} kW | FP atual = ${(fpAtual * 100).toFixed(1)}% | Meta = ${(fpDesejado * 100).toFixed(0)}% | kVAr = P × (tanφ1 - tanφ2) = ${kvarAutomatico.toFixed(1)} kVAr.` +
        (demandaPersonalizadaKw > 0 ? ` (Valor personalizado: ${demandaPersonalizadaKw} kW)` : ` (Demanda registrada: ${demandaReal.toFixed(1)} kW)`) +
  ...
  const exportMemorial = async () => {
    ...
    try {
      ...
      pdf.save(
        `Dimensionamento_Capacitor_${empresaNome.replace(/\s/g, "_")}_${new Date().toISOString().slice(0, 10)}.pdf`,
      );
      Swal.close();
      Swal.fire({
        title: "PDF gerado!",
        text: `Memorial exportado em ${page} página(s).`,
        icon: "success",
      });
    } catch (error) {
      Swal.close();
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      <header className="text-center">
        ...
        <p className="text-slate-500 mt-2">
          Análise baseada em faturas - {empresaNome}
        </p>
        <p className="text-xs text-slate-400 mt-1">
          ...
          {transformadores
            ...
            .join(" + ")}{" "}
          | {transformadores[0]?.tensao_v || 380}V
        </p>
      </header>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Coluna esquerda – configurações (mesmo código original, omitido por brevidade, mas completo no repositório) */}
                ...
                return (
                  <div key={fat.id} className="p-3 rounded-lg bg-slate-50">
                        ...
                        <button
                          onClick={() => {
                            setCurrentFatura({
                              ...
                              consumo_ponta_str:
                                fat.consumo_ponta_kwh.toString(),
                              consumo_fora_str:
                                fat.consumo_fora_ponta_kwh.toString(),
                              demanda_ponta_str:
                                fat.demanda_ponta_kw.toString(),
                              demanda_fora_str:
                                fat.demanda_fora_ponta_kw.toString(),
                              reativo_ponta_str:
                                fat.reativo_ponta_kvarh.toString(),
                              reativo_fora_str:
                                fat.reativo_fora_ponta_kvarh.toString(),
                    ...
                    <div className="grid grid-cols-2 gap-1 text-xs mt-2">
                      <div>
                        Consumo Ponta: {formatNumber(fat.consumo_ponta_kwh, 2)}{" "}
                        kWh
                      </div>
                      <div>
                        Consumo F/Ponta:{" "}
              ...
              <div className="mt-3 space-y-4 p-3 bg-slate-50 rounded-lg">
                <div>
                  <label className="text-xs text-slate-600">
                    Demanda real de pico (kW) – preencha para ignorar
                    estimativas
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) =>
                      setDemandaPersonalizadaKw(parseFloat(e.target.value) || 0)
                    }
                    placeholder="Ex: 140"
                    className="w-full border rounded px-3 py-2 text-sm mt-1"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">
                    Se preenchido (&gt;0), este valor será usado como potência
                    ativa para dimensionamento. Deixe 0 (zero) para usar o
                    cálculo automático.
                  </p>
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setMargemSeguranca(parseFloat(e.target.value) || 0)
                    }
                    placeholder="Ex: 15"
                    className="w-full border rounded px-3 py-2 text-sm mt-1"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">
                    Acrescenta um percentual à potência ativa final (recomendado
                    10-20% para cargas futuras).
                  </p>
                </div>
                <div>
                  <label className="text-xs text-slate-600">
                    Fator de Carga (carga média / potência instalada)
                  </label>
                  <input
                    type="range"
                    min="0.3"
                  ...
                  <p className="text-[10px] text-slate-500">
                    Usado apenas quando a demanda personalizada não é fornecida.
                  </p>
                </div>
                <div>
                  <label className="text-xs text-slate-600">
                    Número de estágios automáticos (6 a 12)
                  </label>
                  <input
                    type="range"
                    min="6"
                    ...
                    onChange={(e) =>
                      setNumeroEstagios(parseInt(e.target.value))
                    }
                    className="w-full"
                  />
                  <div className="flex justify-between text-xs">
          ...
          {result ? (
                    ...
                    <>
                      <div className="text-center border-b pb-4">
                        ...
                        <p className="text-3xl font-bold text-primary">
                          {formatNumber(result.banco_automatico_kvar, 1)} kVAr
                        </p>
                        <p className="text-xs text-slate-400">
                          Banco automático com{" "}
                          {result.estagios_automaticos.length} estágios
                        </p>
                        <p className="text-xs text-slate-400">
                        ...
                        <p className="text-xs text-slate-400">
                          Potência ativa usada:{" "}
                          {result.potencia_ativa_utilizada_kw.toFixed(1)} kW
                        </p>
                      </div>
                      ...
                      {result.pior_mes && (
                        <div className="bg-amber-50 p-4 rounded-xl">
                          ...
                          <p className="text-sm mt-1">
                            FP:{" "}
                            {result.pior_mes.fp_calculado
                              ? (result.pior_mes.fp_calculado * 100).toFixed(1)
                              : "0"}
                            % • Multa:{" "}
                            {formatMoney(
                      ...
                      <div className="bg-indigo-50 p-4 rounded-xl">
                        <p className="text-xs font-bold flex gap-2">
                          <Factory size={14} /> Distribuição do Banco entre
                          Transformadores
                        </p>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-center mt-2">
                          <div className="bg-white rounded p-2 border">
                            ...
                            <p className="font-bold text-lg">
                              {formatMoney(result.investimento_estimado_total)}
                            </p>
                          </div>
                          <div className="bg-white rounded p-2 border">
                            <p className="text-[10px] text-slate-500">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                          <p className="text-xs font-bold text-red-700">
                            a) Prejuízo acumulado
                          </p>
                          <p className="text-xl font-black text-red-700 mt-1">
                            {formatMoney(result.prejuizo_acumulado)}
                          </p>
                          <p className="text-[10px] text-red-600 mt-1">
                            Soma das multas das faturas analisadas.
                          </p>
                        </div>
                        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                          <p className="text-xs font-bold text-blue-700">
                            b) Projeção de economia
                          </p>
                          <div className="text-[11px] mt-2 space-y-1">
                            <p>
                              <strong>1 ano:</strong>{" "}
                              {formatMoney(result.projecao_1_ano)}
                            </p>
                            <p>
                              <strong>3 anos:</strong>{" "}
                              {formatMoney(result.projecao_3_anos)}
                            </p>
                            <p>
                              <strong>5 anos:</strong>{" "}
                              {formatMoney(result.projecao_5_anos)}
                            </p>
                          </div>
                        </div>
                        <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                          ...
                          <p
                            className={`text-xl font-black mt-1 ${result.roi_5_anos_percent >= 0 ? "text-green-700" : "text-red-700"}`}
                          >
                            {formatNumber(result.roi_5_anos_percent, 1)}%
                          </p>
                          <p className="text-[10px] text-green-700 mt-1">
                            Indicador de viabilidade financeira do projeto.
                          </p>
                        </div>
                      </div>
                      <div className="bg-white border rounded-xl p-4">
                        <p className="text-xs font-bold">
                          Resumo executivo (proposta comercial)
                        </p>
                        <p className="text-sm mt-2 text-slate-700">
                          Com base no diagnóstico do fator de potência e no
                          histórico de multas, recomenda-se a implantação de
                          banco de capacitores automático para mitigar perdas
                          financeiras recorrentes. A solução projeta redução
                          relevante das penalidades por energia reativa, com
                          retorno estimado em{" "}
                          <strong>{result.payback_meses} meses</strong> e ROI de{" "}
                          <strong>
                            {formatNumber(result.roi_5_anos_percent, 1)}%
                          </strong>{" "}
                          em 5 anos.
                      ...
                      <div className="bg-slate-50 p-4 rounded-xl">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          ...
                          <div className="col-span-2 text-[10px] text-slate-500 mt-1">
                            • Compatível com rede 3~ 380V • Conformidade NBR
                            14922/2022
                          </div>
                        </div>
                      </div>
            ...
            <div className="h-[500px] flex flex-col items-center justify-center text-center p-8 bg-slate-50 rounded-2xl border-2 border-dashed">
              ...
              <p className="text-sm text-slate-400 mt-2">
                Configure transformadores e adicione faturas para iniciar
              </p>
            </div>
          )}
        </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCurrentFatura({
                          ...currentFatura,
                          consumo_ponta_str: e.target.value,
                        })
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCurrentFatura({
                          ...currentFatura,
                          consumo_fora_str: e.target.value,
                        })
                      }
                      className="w-full border rounded p-2 text-sm mt-1"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCurrentFatura({
                          ...currentFatura,
                          demanda_ponta_str: e.target.value,
                        })
                      }
                      className="w-full border rounded p-2 text-sm mt-1"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCurrentFatura({
                          ...currentFatura,
                          demanda_fora_str: e.target.value,
                        })
                      }
                      className="w-full border rounded p-2 text-sm mt-1"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCurrentFatura({
                          ...currentFatura,
                          reativo_ponta_str: e.target.value,
                        })
                      }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCurrentFatura({
                          ...currentFatura,
                          reativo_fora_str: e.target.value,
                        })
                      }
                      className="w-full border rounded p-2 text-sm mt-1 border-red-200"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCurrentFatura({
                          ...currentFatura,
                          total_pagar_str: e.target.value,
                        })
                      }
                      className="w-full border rounded p-2 text-sm mt-1"
                    />
