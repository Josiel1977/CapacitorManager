...
function DimensionarContent() {
    ...
    const {
      ...
      if (event === "SIGNED_IN" && session && isMounted) {
        ...
        if (profile?.tenant_id) {
          ...
          Swal.fire("Erro", "Perfil não configurado.", "error");
        }
        setCarregando(false);
  ...
  const salvarTransformadores = async () => {
    if (!tenantId) return;
    const trafosComTenant = transformadores.map((t) => ({
      ...t,
      tenant_id: tenantId,
    }));
    const { error } = await supabase
      .from("transformadores")
      .upsert(trafosComTenant, { onConflict: "id" });
    if (error) Swal.fire("Erro", "Não foi possível salvar.", "error");
    else Swal.fire("✅ Sucesso!", "Configuração salva!", "success");
  };
  ...
  const calcularDimensionamento = () => {
    ...
    try {
      ...
      let potenciaAtivaFinal: number;
      if (demandaPersonalizadaKw > 0) {
        potenciaAtivaFinal = demandaPersonalizadaKw;
      } else {
        const potenciaInstaladaKVA = potenciaTotalTransformadores;
        const potenciaAtivaEstimada =
          potenciaInstaladaKVA * fatorCarga * fpAtual;
        potenciaAtivaFinal = Math.max(
      ...
      if (precisaCapacitor) {
        ...
        motivo =
          ...
          (demandaPersonalizadaKw > 0
            ? ` (Valor personalizado informado: ${demandaPersonalizadaKw} kW)`
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-red-50 p-4 text-center rounded-xl">
                          ...
                          <p className="text-2xl font-bold">
                            {formatMoney(result.multa_atual_mensal_real)}
                          </p>
                          <p className="text-xs text-red-500 mt-1">
                            Calculada a partir do reativo excedente
                          </p>
                        </div>
                      ...
                      <div>
                        <h3 className="font-bold mb-2 flex gap-2">
                          <Layers size={18} /> Estágios do Banco (
                          {result.estagios_automaticos.length} estágios)
                        </h3>
                        <div className="flex flex-wrap gap-2">
                          {result.estagios_automaticos.map((s, i) => (
                            <div
                              key={i}
