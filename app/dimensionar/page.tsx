...
import {
  ...
  AlertTriangle, Package, History, Printer, Activity, Plus,
...
interface Fatura {
  id: string; mes_referencia: string; consumo_ponta_kwh: number; consumo_fora_ponta_kwh: number;
  demanda_ponta_kw: number; demanda_fora_ponta_kw: number; reativo_ponta_kvarh: number;
  reativo_fora_ponta_kvarh: number; total_pagar: number; dias_ciclo: number; concessionaria: string;
  tenant_id?: string; fp_calculado?: number;
}
...
interface ResultadoDimensionamento {
  banco_automatico_kvar: number; estagios_automaticos: number[]; tensao_capacitores: string;
  fator_dessintonia: number; economia_mensal_estimada: number; investimento_estimado_total: number;
  payback_meses: number; fp_atual_percent: number; fp_projetado_percent: number;
  multa_atual_mensal_real: number; potencia_ativa_utilizada_kw: number; precisa_capacitor: boolean;
  grupo_tarifario: "A" | "B"; motivo_recomendacao: string; concessionaria_identificada: string;
  quantidade_faturas_analisadas: number; pior_mes: Fatura | null;
  media_fp_por_mes: Array<{ mes: string; fp: number; multa: number }>; alertas: string[];
  distribuicao_por_trafo: DistribuicaoTrafo[]; fornecedores_recomendados: typeof FORNECEDORES_RECOMENDADOS;
  preco_por_kvar: number; economia_anual: number; retorno_5_anos: number; prejuizo_acumulado: number;
  projecao_1_ano: number; projecao_3_anos: number; projecao_5_anos: number; roi_5_anos_percent: number;
  metodo_calculo_utilizado: string; fator_carga_utilizado: number; numero_estagios: number;
}
...
const distribuirKvarPorTrafo = (transformadores: Transformador[], estagiosGlobais: number[], kvarTotal: number): DistribuicaoTrafo[] => {
  ...
  return transformadores.map((trafo) => {
    ...
    if (Math.abs(diff) > 0.01) {
      ...
      estagiosProporcionais[estagiosProporcionais.length - 1] = Math.max(2.5, Math.ceil(estagiosProporcionais[estagiosProporcionais.length - 1] / 2.5) * 2.5);
    }
    ...
    return { trafo_kva: potenciaTrafo, percentual: percentual * 100, kvar_recomendado: kvarRecomendado, kvar_comercial: kvarComercial, preco_estimado: precoEstimado, configuracao_estagios: configuracao };
  });
};
...
function DimensionarContent() {
  ...
  // Autenticação (resumida, funcional)
  useEffect(() => {
    let mounted = true;
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "SIGNED_IN" && session && mounted) {
        setTenantId(null); setCarregando(true);
        ...
        setCarregando(false); setTenantId(null); setFaturas([]); setTransformadores([]);
      }
    });
    (async () => {
  ...
  const carregarDados = async (tenant: string) => {
    try {
      ...
      if (faturasDB?.length) setFaturas(faturasDB);
        ...
        const faturasRaw = [
          { id: crypto.randomUUID(), mes_referencia: "11/2025", consumo_ponta_kwh: 457.21, consumo_fora_ponta_kwh: 5179.86, demanda_ponta_kw: 53.42, demanda_fora_ponta_kw: 53.42, reativo_ponta_kvarh: 493.76, reativo_fora_ponta_kvarh: 4696.54, total_pagar: 12617.5, dias_ciclo: 30, concessionaria: "EQUATORIAL_PARA", tenant_id: tenant },
          { id: crypto.randomUUID(), mes_referencia: "12/2025", consumo_ponta_kwh: 595.56, consumo_fora_ponta_kwh: 6106.21, demanda_ponta_kw: 40.66, demanda_fora_ponta_kw: 40.66, reativo_ponta_kvarh: 1130.49, reativo_fora_ponta_kvarh: 8932.83, total_pagar: 14486.71, dias_ciclo: 31, concessionaria: "EQUATORIAL_PARA", tenant_id: tenant },
          { id: crypto.randomUUID(), mes_referencia: "01/2026", consumo_ponta_kwh: 558.52, consumo_fora_ponta_kwh: 5974.5, demanda_ponta_kw: 37.96, demanda_fora_ponta_kw: 39.98, reativo_ponta_kvarh: 993.0, reativo_fora_ponta_kvarh: 8690.47, total_pagar: 13728.12, dias_ciclo: 31, concessionaria: "EQUATORIAL_PARA", tenant_id: tenant }
        ];
        await supabase.from("faturas").insert(faturasRaw);
        setFaturas(faturasRaw);
      }
    } catch (error: any) { Swal.fire("Erro", error.message || "Falha ao carregar dados.", "error"); } finally { setCarregando(false); }
  };

  // CRUD Transformadores
  const salvarTransformadores = async () => { if (!tenantId) return; const { error } = await supabase.from("transformadores").upsert(transformadores.map(t => ({ ...t, tenant_id: tenantId })), { onConflict: "id" }); error ? Swal.fire("Erro", "Não foi possível salvar.", "error") : Swal.fire("✅ Sucesso!", "Configuração salva!", "success"); };
  ...
  const removerTransformador = async (idx: number) => { if (transformadores.length <= 1) return; const removido = transformadores[idx]; setTransformadores(transformadores.filter((_, i) => i !== idx)); if (removido.id && !removido.id.startsWith("temp_")) await supabase.from("transformadores").delete().eq("id", removido.id); };
  ...
  const salvarFatura = async () => {
    ...
    setFaturas(novas); setShowFaturaModal(false); setCurrentFatura({}); setEditandoFaturaId(null);
  ...
  const removerFatura = async (id: string) => {
    ...
    if (error) Swal.fire("Erro", "Não foi possível remover.", "error"); else setFaturas(faturas.filter(f => f.id !== id));
  };

  // Dimensionamento (com margem de segurança e demanda personalizada)
  const calcularDimensionamento = () => {
    if (faturas.length < 2) { Swal.fire("Atenção", "Mínimo de 2 faturas", "warning"); return; }
    setCalculando(true);
    try {
      ...
      let potenciaBase = demandaPersonalizadaKw > 0 ? demandaPersonalizadaKw : Math.max(demandaMaxRegistrada, potenciaTotalTransformadores * fatorCarga * fpAtual);
      ...
      if (precisaCapacitor) {
        ...
        motivo = `Potência ativa utilizada = ${potenciaAtivaFinal.toFixed(1)} kW | FP atual = ${(fpAtual * 100).toFixed(1)}% | Meta = ${(fpDesejado * 100).toFixed(0)}% | kVAr = P × (tanφ1 - tanφ2) = ${kvarAutomatico.toFixed(1)} kVAr.` +
      ...
      setResult({
        banco_automatico_kvar: kvarAutomatico, estagios_automaticos: estagios, tensao_capacitores: tensaoCapacitores,
        fator_dessintonia: CONFIG_CAPACITORES.dessintonia_padrao, economia_mensal_estimada: economiaMensal,
        investimento_estimado_total: investimentoTotal, payback_meses: payback, fp_atual_percent: fpAtual * 100,
        fp_projetado_percent: precisaCapacitor ? fpDesejado * 100 : fpAtual * 100, multa_atual_mensal_real: mediaMulta,
        potencia_ativa_utilizada_kw: potenciaAtivaFinal, precisa_capacitor: precisaCapacitor, grupo_tarifario: grupoTarifario,
        motivo_recomendacao: motivo, concessionaria_identificada: concessionarias[0] || "NÃO IDENTIFICADA",
        quantidade_faturas_analisadas: faturasProcessadas.length, pior_mes: piorMes || null, media_fp_por_mes: mediaFpPorMes,
        alertas: alertas, distribuicao_por_trafo: distribuicaoPorTrafo, fornecedores_recomendados: FORNECEDORES_RECOMENDADOS,
        preco_por_kvar: precoPorKvar, economia_anual: economiaAnual, retorno_5_anos: retorno5Anos, prejuizo_acumulado: prejuizoAcumulado,
        projecao_1_ano: projecao1Ano, projecao_3_anos: projecao3Anos, projecao_5_anos: projecao5Anos, roi_5_anos_percent: roi5AnosPercent,
        metodo_calculo_utilizado: "Fórmula clássica P×Δtan", fator_carga_utilizado: fatorCarga, numero_estagios: numeroEstagios
      });
      Swal.fire({ title: precisaCapacitor ? "✅ Dimensionamento Concluído" : "✅ Análise Concluída", html: `<div class="text-center"><p class="text-lg font-bold">FP no pior mês: ${(fpAtual * 100).toFixed(1)}%</p>${precisaCapacitor ? `<p class="text-primary font-bold mt-2">🔋 Recomendação:<br/>• Banco automático: ${kvarAutomatico.toFixed(1)} kVAr (${estagios.length} estágios)</p>` : '<p class="text-green-600 mt-2">Sistema dentro das normas ANEEL</p>'}<p class="text-xs text-slate-500 mt-2">💰 Multa média: ${formatMoney(mediaMulta)}/mês</p><p class="text-xs text-slate-500">💰 Investimento estimado: ${formatMoney(investimentoTotal)}</p><p class="text-xs text-slate-500">⏱️ Payback: ${payback} meses</p></div>`, icon: precisaCapacitor ? "success" : "info", timer: 6000 });
    } catch (error) { console.error(error); Swal.fire("Erro", "Falha ao processar dimensionamento", "error"); } finally { setCalculando(false); }
  };
  ...
  const exportMemorial = async () => {
    ...
    try {
      ...
      while (heightLeft > 0) { position -= pdfHeight; pdf.addPage(); pdf.addImage(dataUrl, "PNG", 10, position, pdfWidth, imgHeight); heightLeft -= pdfHeight; page++; }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      <header className="text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">Infraestrutura: {transformadores.map(t => `${t.quantidade}x${t.potencia_kva}kVA`).join(" + ")} | {transformadores[0]?.tensao_v || 380}V</p>
      </header>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={salvarTransformadores} className="text-xs bg-primary text-white px-3 py-1 rounded-lg"><Save size={12} /> Salvar</button>
            </div>
                ...
                <div className="p-6 space-y-6">
                  {result.precisa_capacitor ? (
                    <>
                      ...
                      {result.pior_mes && <div className="bg-amber-50 p-4 rounded-xl"><p className="text-xs font-bold">Pior Mês: {result.pior_mes.mes_referencia}</p><p className="text-sm mt-1">FP: {((result.pior_mes.fp_calculado || result.pior_mes.fp) * 100).toFixed(1)}% • Multa: {formatMoney(calcularMultaDaFatura(result.pior_mes as Fatura))}</p></div>}
      ...
      {/* Modal de Fatura (idêntico ao anterior, omitido por espaço, mas mantido) */}
      <AnimatePresence>
        {showFaturaModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-3">
                ...
                <div><label className="text-xs text-slate-500">Multa informada (opcional)</label><input type="text" placeholder="Ex: 1961.05" value={currentFatura.multa_informada_str ?? ""} onChange={(e) => setCurrentFatura({ ...currentFatura, multa_informada_str: e.target.value })} className="w-full border rounded p-2 text-sm mt-1" /></div>
              </div>
              <div className="flex gap-3 mt-6"><button onClick={() => setCurrentFatura({ mes_referencia: "05/2025", concessionaria: "RORAIMA_ENERGIA", consumo_ponta_str: "8132", consumo_fora_str: "59050", demanda_ponta_str: "430", demanda_fora_str: "447", reativo_ponta_str: "824", reativo_fora_str: "4511", total_pagar_str: "55970.04", dias_ciclo: 30 })} className="flex-1 py-2 border rounded-lg text-sm hover:bg-slate-50">Exemplo Roraima</button><button onClick={salvarFatura} className="flex-1 py-2 bg-primary text-white rounded-lg text-sm font-medium">Salvar Fatura</button></div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
