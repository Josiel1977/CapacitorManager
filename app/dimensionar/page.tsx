...
> = {
  ...
  "160": {
    preco_medio: 26480,
    faixa_preco: "R$ 24.500 - R$ 28.000",
    fornecedores: ["WEG", "Startek"],
  },
  "180": {
    preco_medio: 26900,
...
const calcularFatorPotencia = (
  ...
  return aparente === 0
    ? 0.92
    : Math.min(0.99, Math.max(0.3, ativo_kwh / aparente));
};
...
const calcularMultaDaFatura = (fatura: Fatura): number => {
  const reativoExcedenteTotal =
    fatura.reativo_ponta_kvarh + fatura.reativo_fora_ponta_kvarh;
  const tarifa =
    TARIFAS_REATIVO[fatura.concessionaria as keyof typeof TARIFAS_REATIVO] ??
...
const calcularPrecoMercado = (kvar: number): number => {
  ...
  return kvar !== maisProximo
    ? Math.round(kvar * (preco / maisProximo))
    : preco;
};
...
export default function DimensionarPage() {
  ...
  const carregarFaturaExemploReal = () => {
    ...
    localStorage.setItem(
      "dimensionar_faturas",
      JSON.stringify(faturasComMulta),
    );
  };
  ...
  const salvarFatura = async () => {
    ...
    let novasFaturas = editandoFaturaId
  ...
  const adicionarTransformador = () =>
    setTransformadores([
      ...transformadores,
      {
        id: (transformadores.length + 1).toString(),
        potencia_kva: 100,
        quantidade: 1,
        tensao_v: 220,
        horas_trabalho: 220,
  ...
  const removerTransformador = (idx: number) => {
  ...
  const calcularDimensionamento = () => {
    ...
    try {
      ...
      const potenciaAtivaFinal = Math.max(
      ...
      setResult({
        ...
        pior_mes: piorMes
          ? { ...piorMes, multa_calculada: piorMes.multa }
          : null,
              ...
              <div className="mt-3 space-y-4 p-3 bg-slate-50 rounded-lg">
                <div>
                  ...
                  <p className="text-[10px] text-slate-500">
                    Valor típico para armazéns: 0,65. Aumente se a carga média
                    for maior.
                  </p>
                </div>
                <div>
                  <label className="text-xs text-slate-600">
                    kVAr Recomendado Manual (opcional)
                  </label>
                  <div className="flex gap-2 mt-1">
                    ...
                    />
                    <button
                      onClick={() => setKvarManual(160)}
                      className="px-2 bg-primary text-white rounded text-xs whitespace-nowrap"
                    >
                      USAR VALOR STARTEK (160)
                    </button>
                  </div>
                  <p className="text-[10px] text-slate-500 mt-1">
                    Se preenchido, substitui o cálculo automático.
                  </p>
                </div>
              </div>
            </details>
          ...
          {result ? (
                  ...
                  {result.precisa_capacitor ? (
                    <>
                      <div className="text-center border-b pb-4">
                        ...
                        <p className="text-xs text-slate-400">
                          Fator de carga utilizado:{" "}
                          {result.fator_carga_utilizado.toFixed(2)}
                        </p>
                      </div>
                      ...
                      {result.pior_mes && (
                        <div className="bg-amber-50 p-4 rounded-xl">
                          ...
                          <p className="text-sm mt-1">
                            ...
                            % • Multa:{" "}
                            {formatMoney(result.pior_mes.multa_calculada || 0)}
                          </p>
                        </div>
                      )}
            ...
            <div className="h-[500px] flex flex-col items-center justify-center text-center p-8 bg-slate-50 rounded-2xl border-2 border-dashed">
              ...
              <p className="text-sm text-slate-400 mt-2">
                Configure transformadores e adicione faturas da Equatorial Pará
                ou Roraima
              </p>
              <button
                onClick={carregarFaturaExemploReal}
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs text-slate-500">
                    Multa informada (opcional, substitui a calculada)
                  </label>
                  <input
                    type="text"
                    placeholder="Ex: 1961,05"
                    value={
                      currentFatura.multa_informada !== undefined
                        ? formatMoney(currentFatura.multa_informada as number)
                        : ""
                    }
                    onChange={(e) =>
                      setCurrentFatura({
                        ...currentFatura,
                        multa_informada: parseBRLocal(e.target.value),
                      })
                    }
                    className="w-full border rounded p-2 text-sm mt-1"
                  />
