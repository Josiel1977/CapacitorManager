              ...
              <div ref={reportRef} className="bg-white rounded-2xl overflow-hidden shadow-sm border">
                ...
                <div className="p-6 space-y-6">
                  {result.precisa_capacitor ? (
                    <>
                      <div className="text-center border-b pb-4"><p className="text-sm text-slate-500">Solução Proposta</p><p className="text-2xl font-bold text-primary">Banco fixo (célula capacitiva): {formatNumber(result.banco_fixo_kvar, 1)} kVAr</p><p className="text-2xl font-bold text-primary">Banco automático: {formatNumber(result.banco_automatico_kvar, 1)} kVAr ({result.estagios_automaticos.length} estágios)</p><p className="text-lg font-bold text-primary mt-2">Total: {formatNumber(result.banco_fixo_kvar + result.banco_automatico_kvar, 1)} kVAr</p><p className="text-xs text-slate-400">Grupo {result.grupo_tarifario} • {result.quantidade_faturas_analisadas} faturas • Método: {result.metodo_calculo_utilizado}</p><p className="text-xs text-slate-400">Fator de carga: {result.fator_carga_utilizado.toFixed(2)} • Correção fixa: {result.correcao_fixa_percent}%</p></div>
