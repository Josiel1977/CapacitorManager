...
const calcularKvarNecessario = (
  ...
  const fp_desejado_seguro = Math.min(
    0.99,
    Math.max(fp_desejado, FP_MINIMO_REGULAMENTAR),
  );
...
export default function DimensionarPage() {
  ...
  const calcularDimensionamento = () => {
    ...
    try {
      ...
      const correcaoFixaKvar =
        (potenciaTotalTransformadores * correcaoFixaPercent) / 100;
      ...
      const precoPorKvar =
        kvarTotalComercial > 0
          ? investimentoAutomatico / kvarTotalComercial
          : 0;
      ...
      setResult({
        ...
        metodo_calculo_utilizado:
          "Fórmula clássica P×Δtan + célula capacitiva fixa",
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border">
            ...
            <details className="mb-4">
              ...
              <div className="mt-3 space-y-4 p-3 bg-slate-50 rounded-lg">
                ...
                <div>
                  <label className="text-xs text-slate-600">
                    Correção fixa (célula capacitiva) - % da potência dos
                    transformadores
                  </label>
                  <input
                    type="range"
                    ...
                    max="10"
                    step="1"
                    value={correcaoFixaPercent}
                    onChange={(e) =>
                      setCorrecaoFixaPercent(parseInt(e.target.value))
                    }
                    className="w-full"
                  />
                  <div className="flex justify-between text-xs">
                    <span>0%</span>
                    <span className="font-bold">{correcaoFixaPercent}%</span>
                    <span>10%</span>
                  </div>
                  <p className="text-[10px] text-slate-500">
                    Recomendado 3-5% para compensar o reativo fixo do
                    transformador.
                  </p>
                </div>
                <div>
                    ...
                    <>
                      <div className="text-center border-b pb-4">
                        ...
                        <p className="text-2xl font-bold text-primary">
                          ...
                          {formatNumber(result.banco_fixo_kvar, 1)} kVAr
                        </p>
                        <p className="text-2xl font-bold text-primary">
                          Banco automático:{" "}
                        ...
                        <p className="text-lg font-bold text-primary mt-2">
                          Total:{" "}
                          {formatNumber(
                            result.banco_fixo_kvar +
                              result.banco_automatico_kvar,
                            1,
                          )}{" "}
                          kVAr
                        </p>
                        ...
                        <p className="text-xs text-slate-400">
                          Fator de carga:{" "}
                          {result.fator_carga_utilizado.toFixed(2)} • Correção
                          fixa: {result.correcao_fixa_percent}%
                        </p>
                      </div>
                      ...
                      <div className="bg-indigo-50 p-4 rounded-xl">
                        <p className="text-xs font-bold flex gap-2">
                          <Factory size={14} /> Distribuição do Banco Automático
                          ...
                          >
                            ...
                            <div className="grid grid-cols-2 gap-1 text-sm mt-2">
                              ...
                              <div className="col-span-2 font-medium">
                                ...
                                {formatMoney(dist.preco_estimado)}
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      <div>
                        <h3 className="font-bold mb-2 flex gap-2">
                          <Layers size={18} /> Estágios do Banco Automático (
                          {result.estagios_automaticos.length} estágios)
                        </h3>
