...
import {
  Calculator, Zap, TrendingUp, DollarSign, CheckCircle2, FileText,
  Loader2, AlertTriangle, Package, History, Printer, Activity, Layers,
  Plus, Trash2, Save, Edit3, X, AlertCircle, Factory, Truck,
} from "lucide-react";
...
const formatNumber = (valor: number, decimals: number = 2): string => {
  return new Intl.NumberFormat("pt-BR", { minimumFractionDigits: decimals, maximumFractionDigits: decimals }).format(valor);
};
...
export default function DimensionarPage() {
  ...
  const validarFatura = (fatura: Partial<Fatura>): { valida: boolean; mensagem: string } => {
    ...
    if (reativoTotal > consumoTotal && consumoTotal > 0) {
      ...
      if (percentual > 200) return { valida: false, mensagem: `Reativo é ${percentual.toFixed(0)}% maior que o Consumo. Verifique se os valores estão trocados.` };
    }
    const fp = calcularFatorPotencia(consumoTotal, reativoTotal);
    if (fp < 0.5 && consumoTotal > 0 && reativoTotal > 0) return { valida: false, mensagem: `FP calculado é ${(fp*100).toFixed(1)}%. Verifique os dados.` };
  ...
  const salvarFatura = async () => {
    ...
    const novaFatura: Fatura = {
      ...
      validado: true,
      fp_calculado: fpCalc,
      fp_informado: currentFatura.fp_informado ? parseBRLocal(currentFatura.fp_informado) : undefined,
  ...
  const removerFatura = (id: string) => {
    Swal.fire({ title: "Remover fatura?", text: "Esta ação não pode ser desfeita.", icon: "warning", showCancelButton: true, confirmButtonColor: "#e74c3c", confirmButtonText: "Remover" }).then((result) => {
  ...
  const calcularDimensionamento = () => {
    ...
    try {
      ...
      faturas.forEach(f => {
        console.log(`${f.mes_referencia}: ReativoP=${f.reativo_ponta_kvarh}, ReativoF=${f.reativo_fora_ponta_kvarh}`);
      });
      ...
      const faturasProcessadas = faturas.map(f => {
        ...
        if (reativoTotal === 0 && ativoTotal > 0) alertas.push(`⚠️ Fatura ${f.mes_referencia} com REATIVO ZERO.`);
        ...
        console.log(`Fatura ${f.mes_referencia}: Ativo=${ativoTotal}, Reativo=${reativoTotal}, Multa=${formatMoney(multa)}, Demanda=${demandaMaxKw}`);
      ...
      if (precisaCapacitor) {
        ...
        if (economiaMensal < 100) alertas.push("⚠️ Economia mensal < R$100. Verifique se os reativos estão corretos.");
      } else {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      <header className="text-center">
        ...
        <p className="text-xs text-slate-400 mt-1">Infraestrutura: 1x300kVA + 1x225kVA | 380V</p>
      </header>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setCurrentFatura({}); setEditandoFaturaId(null); setShowFaturaModal(true); }} className="text-xs bg-primary text-white px-3 py-1 rounded-lg"><Plus size={12} /> Adicionar</button>
            </div>
            <div className="max-h-80 overflow-y-auto space-y-2">
              {faturas.length === 0 ? (
                <div className="text-center py-8"><p>Nenhuma fatura</p><button onClick={carregarFaturaExemploReal} className="text-primary text-sm">Carregar exemplo</button></div>
              ) : (
                faturas.map((fat) => {
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border">
            ...
            <select value={targetFP} onChange={(e) => setTargetFP(parseFloat(e.target.value))} className="w-full rounded-xl border p-3 mb-6">
            ...
            <button onClick={calcularDimensionamento} disabled={calculando || faturas.length < 2} className="w-full bg-primary text-white py-3 rounded-xl font-bold disabled:opacity-50 flex justify-center gap-2">{calculando ? <Loader2 className="animate-spin" size={20} /> : <Zap size={20} />} Calcular</button>
          </div>
        </div>
                ...
                <div className="p-6 space-y-6">
                  {result.precisa_capacitor ? (
                    <>
                      ...
                      {result.alertas.map((a,i) => <div key={i} className="bg-amber-50 p-3 rounded-xl text-xs text-amber-700">{a}</div>)}
                      <div className="bg-blue-50 p-4 rounded-xl"><p className="text-sm font-bold text-blue-700">📌 {result.motivo_recomendacao}</p><p className="text-xs">FP atual: {result.fp_atual_percent.toFixed(1)}% → Meta: {result.fp_projetado_percent.toFixed(0)}%</p><div className="mt-3"><BarraFP fp={result.fp_atual_percent} /><div className="flex justify-between text-[10px] mt-1"><span>Atual: {result.fp_atual_percent.toFixed(1)}%</span><span>Meta ANEEL: 92%</span></div></div></div>
                  ...
                  ) : <div className="text-center py-8"><CheckCircle2 size={40} className="mx-auto text-green-600 mb-2" /><p className="text-xl font-bold text-green-700">Instalação Regularizada</p><p className="text-sm">{result.motivo_recomendacao}</p></div>}
                  <div className="text-center text-[10px] text-slate-400 border-t pt-4"><p>Cálculos baseados em ANEEL e NBR 14922/2022</p></div>
                </div>
              </div>
              <button onClick={exportMemorial} className="w-full bg-white border py-3 rounded-xl font-medium flex justify-center gap-2"><Printer size={18} /> Exportar PDF</button>
            </motion.div>
          ) : (
            <div className="h-[500px] flex flex-col items-center justify-center text-center p-8 bg-slate-50 rounded-2xl border-2 border-dashed"><Calculator size={64} className="text-slate-300 mb-4" /><h3 className="text-xl font-bold">Aguardando Dados</h3><p className="text-sm text-slate-400">Configure transformadores e adicione pelo menos 2 faturas</p></div>
          )}
        </div>
      </div>
