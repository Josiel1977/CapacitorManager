...
// import { supabase } from "@/lib/supabase"; // Descomente se usar Supabase
...
interface ResultadoDimensionamento {
  ...
  detalhes_calculo: { potencia_base_kw: number; fator_utilizacao: number; margem_seguranca: number; formula_aplicada: string; };
  alertas: string[];
}
...
const validarESelecionarFP = (fpLido: number | undefined, ativo: number, reativo: number): { fp: number; alerta?: string } => {
  ...
  if (diff > 0.18) {
    return { fp: fpCalc, alerta: `FP lido (${(fpLido*100).toFixed(1)}%) diverge do cálculo energético (${(fpCalc*100).toFixed(1)}%). Usado valor calculado.` };
  }
...
function parseFaturaFromPDF(texto: string): Partial<Fatura> & { concessionaria: string } {
  const dados: any = {
    concessionaria: texto.includes("Equatorial") ? "EQUATORIAL_PARA" : "DESCONHECIDA",
  ...
  const extrairMes = () => {
    ...
    if (matches) {
      for (const d of matches) {
        ...
        if (year >= 2024 && year <= 2030) return d;
      }
    }
    return "";
  };
  ...
  // Demanda Máxima
  const demFP = pegarValor(/Demanda\s*(?:Ativa|Distribui[çc][ãa]o)\s*(?:FP|Reg)[\s\S]*?([\d\.]+,\d+|\d+)\s*kW/i);
  const demNP = pegarValor(/Demanda\s*(?:Ativa|Distribui[çc][ãa]o)\s*(?:NP|Reg)[\s\S]*?([\d\.]+,\d+|\d+)\s*kW/i);
  dados.demanda_max_kw = Math.max(demFP, demNP, 0.01);
...
function DimensionarContent() {
  ...
  const onDropPDF = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      Swal.fire("✅ PDF Importado", `Fatura de ${novaFatura.mes_referencia} processada com sucesso.`, "success");
    } catch (e: any) {
      console.error("Erro parser:", e);
      Swal.fire("Erro na Leitura", e.message || "Formato inválido ou PDF protegido.", "error");
  ...
  const calcular = () => {
    ...
    setTimeout(() => {
      try {
        ...
        const demandaPico = Math.max(...faturas.map(f => f.demanda_max_kw), 0);
        
        let potenciaBase = 0;
        if (modoCalculo === "pico") {
          potenciaBase = demandaPico;
          if (potenciaBase < 10) alertas.push("⚠️ Demanda medida muito baixa. Verifique a fatura ou use modo 'Média'.");
        } else {
          const horasMedia = faturas.reduce((acc, f) => acc + f.dias_ciclo * 24, 0) / faturas.length;
          const ativoMedio = faturas.reduce((acc, f) => acc + f.consumo_ativo_total_kwh, 0) / faturas.length;
          potenciaBase = ativoMedio / horasMedia * 1.2;
        }

        const fpPior = Math.min(...fps);
        const fpMedio = faturas.reduce((a,b) => a + b.fp_calculado, 0) / faturas.length;
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border">
            ...
            <div className="space-y-3 text-sm">
              <div>
                <label className="block text-xs text-slate-500">FP Desejado</label>
                <select value={targetFP} onChange={e => setTargetFP(parseFloat(e.target.value))} className="w-full border rounded p-2 mt-1">
              ...
              <div>
                <label className="block text-xs text-slate-500">Base de Cálculo</label>
                <div className="flex gap-2 mt-1">
                  <button onClick={() => setModoCalculo("pico")} className={`flex-1 py-2 rounded text-xs ${modoCalculo==="pico"?"bg-blue-600 text-white":"bg-slate-100"}`}>Pico Medido</button>
                  <button onClick={() => setModoCalculo("media")} className={`flex-1 py-2 rounded text-xs ${modoCalculo==="media"?"bg-blue-600 text-white":"bg-slate-100"}`}>Média Mensal</button>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs text-slate-500">Margem Seg. (%)</label>
        ...
        <div className="lg:col-span-2">
          {result ? (
              ...
              <div ref={reportRef} className="bg-white rounded-xl p-6 shadow-sm border">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Gerado em {new Date().toLocaleDateString("pt-BR")} • {faturas.length} faturas analisadas</p>
                  </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl mb-4">
                  ...
                  <div className="text-xs text-slate-600 space-y-1 font-mono bg-white p-3 rounded border">
                    <p>• Potência base utilizada: {formatNumber(result.detalhes_calculo.potencia_base_kw, 1)} kW</p>
                    ...
                    <p>• Margem de segurança aplicada: {result.detalhes_calculo.margem_seguranca}%</p>
            ...
            <div className="h-[400px] flex flex-col items-center justify-center border-2 border-dashed rounded-xl bg-slate-50 text-slate-400">
              ...
              <p className="text-sm">Importe faturas em PDF ou adicione manualmente para iniciar</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-xl p-6 w-full max-w-md">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input placeholder="Dias do ciclo" type="number" value={editFatura.dias||30} onChange={e=>setEditFatura({...editFatura, dias:e.target.value})} className="border rounded p-2"/>
                  <input placeholder="FP lido (opcional)" type="number" step="0.01" value={editFatura.fp_lido||""} onChange={e=>setEditFatura({...editFatura, fp_lido:e.target.value})} className="border rounded p-2"/>
                </div>
                <select value={editFatura.concessionaria||"EQUATORIAL_PARA"} onChange={e=>setEditFatura({...editFatura, concessionaria:e.target.value})} className="w-full border rounded p-2">
